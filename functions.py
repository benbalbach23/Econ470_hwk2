...
def read_contract(path):
    ...
    df = pd.read_csv(path, skiprows=1, names=col_names, dtype={
...
def read_enroll(path):
    ...
    df = pd.read_csv(path, skiprows=1, names=col_names, na_values="*", dtype={
...
def read_penetration(path):
    ...
    df = pd.read_csv(path, skiprows=1, names=col_names, na_values=['', 'NA', '*', '-', '--'], dtype={
